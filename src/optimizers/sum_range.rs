...
pub fn optimize(node: &mut Node) -> bool {
    if let Node::Func {
        ...
        && args.len() == 1
    {
        if let Node::Range(start, end) = &args[0]
                ...
                (start.as_ref(), end.as_ref())
        {
            ...
            if predicate.is_none() {
                *node = Node::Value(Value::Number(sum));
                ...
                if let Some(result) = apply_sum_predicate(sum, count, pred) {
...
mod tests {
    ...
    fn sum_range_multiply_predicate() -> Result<()> {
        assert_eq!(eval("sum(1..3, {# * 2})", &Context::default())?.to_string(), "12");
        Ok(())
    }

    #[test]
    fn sum_range_add_predicate() -> Result<()> {
        assert_eq!(eval("sum(1..3, {# + 1})", &Context::default())?.to_string(), "9");
        Ok(())
    }

    #[test]
    fn sum_range_subtract_predicate() -> Result<()> {
        assert_eq!(eval("sum(1..3, {# - 1})", &Context::default())?.to_string(), "3");
        Ok(())
    }

    #[test]
    fn reduce_range_sum() -> Result<()> {
        assert_eq!(eval("reduce(1..5, {# + #acc})", &Context::default())?.to_string(), "15");
        Ok(())
    }
