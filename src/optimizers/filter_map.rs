...
fn contains_index_pointer(node: &Node) -> bool {
    match node {
        ...
        Node::Range(start, end) => contains_index_pointer(start) || contains_index_pointer(end),
        Node::Value(_) => false,
...
fn operator_contains_index(op: &crate::ast::postfix_operator::PostfixOperator) -> bool {
    match op {
        ...
        | crate::ast::postfix_operator::PostfixOperator::Pipe(n) => contains_index_pointer(n),
    ...
    fn ast_filter_map_skips_hash_in_map_predicate() {
        ...
        match &optimized {
            Node::Func { ident, .. } => {
                assert_eq!(
                    ident, "map",
                    "should stay as map when predicate references #"
                );
            }
            other => panic!("Expected Func node, got {other:?}"),
        }
